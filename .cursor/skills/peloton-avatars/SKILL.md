...
**Eyes** (`EYE_RECIPES`): keep a **neutral-open** cluster as the common weight. Billboard eyes (`hw` above ~22, affine `scale_x` past ~1.03, or pair spacing past ±5 px) read as too wide. Live `hw` is **21.6–22.0**, baker cap **22.0**, so one eye is about as wide as the gap between them. Flattened lids (`th` under ~12, `bh` under ~10) read as slits — the owner asked to open them **up and down**, not sideways. Live aperture is **th 12.0–14.6 / bh 10.2–12.4**, baker-clamped. Affine `scale_x` is 0.97–1.02; `scale_y` is **1.00–1.10** (never squash); pair `dx` is ±5 px. Vary **lid** (`peak`, `hood`, `flat`, `lash`), **tilt / inner_y / outer_y**, **aperture** within that tall band, and **iris**. Retired ids keep `weight: 0`.
