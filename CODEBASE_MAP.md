...
|---|---|
| `src/Peloton.Domain` | World root, stable IDs, people, `RiderCareer`, ManagerCareer, Employment, Organization, DecisionAuthority, AccessContext, `CalendarEntry`, content/rules identity, DecisionTrace / Spy sinks. |
...
| `src/Peloton.Persistence` | SQLite schema version 2, verified candidate save, envelope identity, snapshot round trip (including `RiderCareer` + results), integrity checks. |
...
| Race preparation | `Peloton.Application/RacePreparation.cs`, `WorldRaceScenarioAssembler.cs`, `GameApplication.cs` | world roster + route template (`D-036` phase 1–2); readiness scales CP/Pmax at assemble; `RaceScenarioId` is route/tuning only | Application + Persistence tests |
| RiderCareer / world–race bind | `Peloton.Domain/RiderCareer.cs`, `WorldRaceScenarioAssembler.cs`, `content/peloton.skeleton/skeleton-roster.json` | `CAREER_WORLDTOUR_SLICE_v0.1.md` phase 1–2; official start lists from world `RiderCareer` rows; day-state rest/race load | `CareerWorldTourBindTests`, `CareerWorldTourPhase2Tests` |
