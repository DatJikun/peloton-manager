...
| `src/Peloton.Client.Godot` | Godot 4.4.1 .NET career shell + optional Watch Race. Presentation only: Commands + Queries. Main scene `CareerShell.tscn` (`CareerShellScreen` chrome + `CareerShellHost` commands + `CareerShellViews` per section). `LookChrome` / `LookIcons` / `LookFormat` / `LookCharts` = CSS-equivalent of HTML v3 (body 14 / meta 10 / Anton 30+26, `stretch=canvas_items` on 1600×900, flat `Table`, `SectionBar`, `Pill`, `NavItem` icons, `Crest`, `ManagerFoot`, `Stat`, `ContractFrame`, `DateChip`). Simulate uses today's world race (`TryGetTodaysRaceContentId`), not the prototype id. Desk / Skład / Rynek / Finanse / Kalendarz / Wynik speak HTML v3; New Game / season plan / Settings inherit chrome only. Staff/sponsors/scouting stay `CareerLookCatalog`. Watch film off by default (D-043 / D-048). Career Hub deleted. File-by-file + VM screenshot recipe: `docs/tasks/2026-09-02-session-handoff-main-agent.md` §4. |
...
| `tests/Peloton.Client.Godot.Tests` | Career shell host (Advance Day / Race next / save-load / default simulate → results table + team filter; WT today's-race id), look catalog, `LookFormat`, optional Watch film, interpolator, film duration, map profile sampling. Compiles Godot-free host + catalog sources; does not need the Godot editor. Pixel parity is VM screenshots, not these tests. |
...
| Godot career shell | `src/Peloton.Client.Godot` (`CareerShellHost`, `CareerShellScreen`, `CareerShellViews`, `LookChrome`, `LookIcons`, `LookFormat`, `LookCharts`, `CareerLookCatalog`) | HTML v3 chrome on `main` (`85b4257`); desk/calendar/inbox/squad/market/finance/result; default simulate → result table (D-043); WT simulate uses today's race; look catalog for empty domains; Career Hub deleted (D-048) | `CareerShellHostTests`, `CareerLookCatalogTests`, `LookFormatTests` |
...
| Career day query | `Peloton.Application/CareerDay.cs`, `ClubRosterProjection`, `ClubFinanceProjection`, `RiderPresentationQueries` | `GAME_STATES_v0.1.md` Advance Day; desk primary action (`advance-day` / `race-next`); employer roster wages via `ClubRosterProjection` plus nationality/age/role/form/`IdentityLine`; employer cash via `ClubFinanceProjection`; Management only; not a UI dashboard | Application tests, `day` SimRunner |
...
| Pre-season planning | `Peloton.Application/PreSeasonPlanning.cs`, `GameApplication.cs` | `PreSeasonPlanningFlow` draft entry + designated leader by `RaceContentId`; `SetSeasonRaceLeaderCommand`; confirm commits `OrganizationRaceEntry` | `CareerWorldTourPhase3Tests`, `CareerClubCalendarLeadersTests` |
| New Game club pick | `GameApplication.ListNewGameClubs`, `CreateWorldCommand` employer arg, `EmployerEligibility` | D-050: 18 WT clubs from `playerStartDivisions`; optional employer on CreateWorld | `CareerClubCalendarLeadersTests` |
| Godot career shell (flow) | `Peloton.Client.Godot/CareerShellHost.cs`, `CareerShellScreen.cs`, `CareerShellViews.cs` | MainMenu club picker → pre-season plan → management; `ClubRosterProjection` on desk/squad; `ClubFinanceProjection` on desk/finance; D-044 contract offers on squad card; world calendar; HTML v3 widgets from `LookChrome` | `CareerShellHostTests`, `LookFormatTests` |
| Race result / debrief | `Peloton.Application/RaceResultDebrief.cs`, `RaceTimePresentationQueries.cs`, `GameApplication.cs` | committed `LastRace` uses world `RiderCareer.Id`; org on `RaceResultPlacement`; finish clock/gap from `RiderStageTime`; `RaceResultForOrganization` filter (D-043); career history append on `RecordRace` | Application + Persistence tests |
...
| Career calendar | `Peloton.Domain/CalendarEntry.cs`, `Peloton.Application/CareerCalendarInbox.cs`, `CourseSparklineQueries.cs` | stored entries + derived status; `CourseSparkline` from `CourseProfile` (24 heights 0–20); `StagesForEvent` | Application tests (`CareerUiQueryTests`, `CareerSeasonEventsTests`) |
...
→ CareerShellScreen.cs / CareerShellViews.cs
→ LookChrome.cs / LookIcons.cs / LookFormat.cs / LookCharts.cs
→ CareerLookCatalog.cs (look only)
→ docs/tasks/2026-09-02-session-handoff-main-agent.md §4.3–4.4 (file map + VM shots)
