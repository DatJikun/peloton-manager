...
| Godot Watch Race | `src/Peloton.Client.Godot` (`WatchRaceHost`, `WatchRaceScreen`) | `D-033` renderer; optional film; `UI_SITEMAP` RaceLive; §49 still owner-only | `WatchRaceHostTests`, `WatchMotionInterpolatorTests`, `WatchFilmDurationTests`, `WatchRouteProfileTests` |
...
Recruitment, transfer market, dynamic sponsors, knowledge records, Career Hub UI (PR #4), and `D-032` multi-stage leadership transfer are not implemented. Godot career shell is the management chrome (D-043 simulate → results). Watch film stays optional. Look-catalog OVR/cash/scouting is presentation fiction, not World State.
