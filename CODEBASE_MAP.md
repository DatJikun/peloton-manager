...
| `src/Peloton.Persistence` | SQLite schema version 6, verified candidate save, envelope identity, snapshot round trip (including `RiderCareer`, `RiderContract`, results, `OrganizationRaceEntry`, org metadata, cash/fee), integrity checks. |
...
| RiderCareer / world–race bind | `Peloton.Domain/RiderCareer.cs`, `RiderContract.cs`, `OrganizationRaceEntry`, `WorldRaceScenarioAssembler.cs`, `content/peloton.skeleton/skeleton-roster.json`, `content/peloton.wt-2026/` | `CAREER_WORLDTOUR_SLICE_v0.1.md` phase 1–6; start lists from entered org rosters (12-rider WT cap); contract expiry on Advance Day; finance tick after expiry | `CareerWorldTourBindTests`, `CareerWorldTourPhase2Tests`, `CareerWorldTourPhase3Tests`, `CareerWorldTourPhase4Tests`, `CareerWorldTourPhase5Tests`, `CareerWorldTourPhase6Tests` |
