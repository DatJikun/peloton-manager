...
Zegar oglądania (Watch Race) jest nadzorujący. W Godocie gracz wybiera **czas filmu** przeciętnego etapu (30 s / 1 min / 2 min / 3 min / **5 min**; domyślnie 5 min, maksimum oglądania). Rate = `clamp(round((TotalLengthM / 6 m/s) / target), 1, 120)`. 6 m/s to tempo filmu z zapasem na podjazd i zmęczenie (żywe km/h na płaskim zostają ~40). Prototyp 5,4 km ≈ 15 min fizyki → ×3 → ok. 5 min oglądania, nie highlight 15 s. Okno Watch: czas filmu, autonomia DS, **Oglądaj** — potwierdzenie planu jest automatyczne. Headless CLI nadal używa jawnego tempa ×1 / ×2 / ×5 / ×20 (`--rate`).
