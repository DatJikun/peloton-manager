...
`src/Peloton.Client.Godot` is presentation only (Commands + Queries). Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET. Main scene: career shell (`CareerShell.tscn`) with Watch Race as an optional overlay. Headless tests do not need Godot installed.
