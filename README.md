...
`src/Peloton.Client.Godot` is presentation only (Commands + Queries). Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET. Main scene: career shell (`CareerShell.tscn`) with Watch Race as an optional overlay, off by default. Career Hub UI is deleted. Headless tests do not need Godot installed.
...
Official results come from `PrototypeRaceEngine`, not a seed-ranked stub. The prototype is still below `RACE_ENGINE_DESIGN_v0.2.md`: one-second `double` steps, simplified shelter/knowledge. Each rider stores two CdA numbers (road vs TT bike, D-055). Selective stages use a pace-setter (D-054). Career races bind to world `RiderCareer` rows; standalone SimRunner `race` still uses the disconnected fixture. Godot Watch exists as an overlay, not the play path (D-043). The owner §49 fun gate is not verified. The Windows playtest zip is published on GitHub Releases, not committed to this repo (D-053). See `KNOWN_DIFFERENCE_FROM_CODE.md`.
