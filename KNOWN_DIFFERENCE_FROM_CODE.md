...
- Jerseys are queries (`ClassificationQueries`): GC, points, KOM, youth, team. SimRunner `day --through-results` prints them. Godot result table has thin jersey lines. Youth eligibility uses `WorldState.SeasonYear` (not a frozen calendar year). D-032 stays deferred.
