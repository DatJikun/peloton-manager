...
- Godot Watch Race exists as an **optional** presentation window over the same D-033 clock (`D-036`); the thin Career Hub default is simulate → official result table, headline events, and a presentation-only team filter (`D-037`), not a KPI dashboard or a fun-gate result;
