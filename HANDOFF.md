...
### Status
Godot Watch Race is a presentation client over the existing Application clock. Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET. The player picks a film length (default **5 min**, max 5 min), optionally gives DS autonomy, and presses **Oglądaj** (plan confirm is automatic). The 5.4 km prototype is ~15 min of physics; ×3 makes a 5 min choice about 5 min of watching (not a 15 s highlight). Then: squad icons, radio board, knowledge-bounded decision (or the DS recommendation in parentheses), official winner from `LastRace`. Renderer does not drive physics. Career shell is not in this window. HTML lab (`08e` / `14-race.html`) stays a look reference, not the game. Headless CLI is unchanged and still uses ×1 / ×2 / ×5 / ×20. §49 remains `NOT VERIFIED`. `D-032` remains deferred. SchemaVersion remains 1.
...
- `2026-08-28` — Watch: czas filmu (domyślnie 5 min, max 5 min) zamiast ×N w Godot; prototyp 5,4 km ogląda się ok. 5 min przy ×3; Oglądaj samo potwierdza plan; mapa z biblioteki profili; tablica radiowa składu; DS (chce: …) i opcjonalna autonomia DS. CLI nadal ×1/×2/×5/×20. Fizyka prototypu bez zmiany goldenu.
