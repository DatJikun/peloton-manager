...
### Status
Godot Watch Race is a presentation client over the existing Application clock. Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET (4.7.2 .NET also works for playtest). The player picks a film length (default **5 min**), optionally gives DS autonomy, and presses **Oglądaj**. Chrome follows `08e` (Anton / PT Sans, red slab buttons, hard black offset). No tutorial/status copy in the window. Then: squad icons, radio board, DS (chce: …) decision buttons that stay clickable, official winner from `LastRace`. Renderer does not drive physics. Career shell is not in this window. HTML lab (`08e` / `14-race.html`) stays a look reference, not the game. Headless CLI is unchanged and still uses ×1 / ×2 / ×5 / ×20. §49 remains `NOT VERIFIED`. `D-032` remains deferred. SchemaVersion remains 1.
