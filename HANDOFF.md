...
- `2026-09-05` — **UI presentation query slice (Application):** `ClubRosterEntry` / `MarketRiderProjection` expose nationality, age, Polish role label, form %, and `IdentityLine`; `CalendarEntryProjection` / `SeasonEventProjection` carry `CourseSparkline` from stored `CourseProfile`; `RaceResultPlacement` joins `RiderStageTime` for finish clock and gap labels; `GameApplication.StagesForEvent` is public for Godot wiring.
