...
### Goal
First playable Watch Race window: prep (film length + optional DS autonomy) → `OGLĄDAJ` auto-confirms the plan → `StartRaceCommand` → D-033 supervising clock, interpolated map icons, decision pause, Results from `LastRace`. Do not close §49.

### Status
Godot Watch Race is a presentation client over the existing Application clock. Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET (4.7.2 .NET also works for playtest). The player picks a film length (default **5 min**), optionally gives DS autonomy, and presses **Oglądaj**. After **Oglądaj**, the live window is a RACE LIVE dashboard (owner mockup): profile with climb shading and gradient legend, squad radio table, race-leader card, time gaps (Zespół / Ogólne), live ticker from real radio. Empty mockup fields stay empty — no invented weather, watts, commentator, or photoreal faces. Header actions are a matched pair (Pauza/Kontynuuj + Wyjdź); the live/pause chip sits on the profile bar, not as a second header button. During a DS decision those playback controls are hidden. Chrome follows `08e` (Anton / PT Sans, red slab buttons, hard black offset). No tutorial/status copy in the window. DS (chce: …) decision buttons stay clickable; official winner from `LastRace`. Renderer does not drive physics. Career shell is not in this window. HTML lab (`08e` / `14-race.html`) stays a look reference, not the game. Headless CLI is unchanged and still uses ×1 / ×2 / ×5 / ×20. §49 remains `NOT VERIFIED`. `D-032` remains deferred. SchemaVersion remains 1.
...
- [x] Godot Watch film duration (30 s–5 min, default 5 min) plus authored route-profile library, squad radio board, DS recommendation button, and optional pre-race DS autonomy
- [x] Godot RACE LIVE dashboard layout (profile / radio table / leader / time gaps / ticker) filled only with prototype Watch facts
...
2026-08-28: tablica sztabu z pozycją i radiem naszych kolarzy; widać różnicę płasko vs góra (km/h z fizyki, czerwony podjazd); film max 5 min, domyślnie 5 min (prototyp 5,4 km ≈ 15 min fizyki, oglądanie ×3 ≈ 5 min); decyzja DS z tym, co chce zrobić; opcjonalna autonomia DS przed startem; Oglądaj samo potwierdza plan.
...
## Handoff summary
Milestone 0 still supplies the headless .NET 8 spine. The race prototype is the official result path: `PrototypeRaceEngine` plus `content/peloton.race-prototype`, Application commands `StartRaceCommand` / `AdvanceRaceCommand` / `RespondToRaceDecisionCommand` / `BeginRaceWatchCommand` / `AdvanceRaceWatchCommand` / `AbandonRaceLiveCommand`, and SimRunner `race`. A pending DecisionRequest stays in `RaceLive`. SimRunner `watch` and career `day --watch-from-prep` keep the D-033 supervising clock. Godot Watch Race (`src/Peloton.Client.Godot`) is presentation only: it issues those commands, interpolates `RaceWatch` snapshots, and shows Results from `LastRace`. Visible in Godot: film duration (default 5 min ≈ 5 min of watching on the 5.4 km prototype), DS autonomy toggle, **Oglądaj** (auto-confirms plan), RACE LIVE dashboard (library profile, squad radio table, leader card, time-gap board, live radio ticker), moving icons, DS (chce: …) decision, winner. No fake commentary/weather/watts. Still CLI-only: Hub/inbox/calendar, 10-year soak, standalone `watch` markdown, Simulate-from-prep. After Simulate/Watch, `RaceResultProjection` and `RaceDebriefProjection` present the committed result without a second `RunBatch`. Spy OFF/ON must match checksum and finish order. `StubRaceEngine` is gone from production assemblies. SQLite `SchemaVersion` remains 1. Owner §49 remains `NOT VERIFIED`. `D-032` is deferred.
...
- `2026-08-28` — Watch: czas filmu (domyślnie 5 min, max 5 min) zamiast ×N w Godot; prototyp 5,4 km ogląda się ok. 5 min przy ×3; Oglądaj samo potwierdza plan; mapa z biblioteki profili; tablica radiowa składu; DS (chce: …) i opcjonalna autonomia DS. Chrome 08e, bez tekstów-poradników. CLI nadal ×1/×2/×5/×20. Fizyka prototypu bez zmiany goldenu.
- `2026-08-28` — Właściciel pokazał mockup RACE LIVE. Godot układa żywy ekran tak samo (profil, zawodnicy, lider, różnice czasowe, pasek komunikatów). Pola, których prototyp nie ma (pogoda, waty, komentator, zdjęcie), zostają puste — nie dopisujemy ich. Pauza/Kontynuuj i Wyjdź to jedna para niebieskich przycisków; status WYŚCIG TRWA jest na belce profilu. Przy decyzji DS pauza/kontynuuj znika.
