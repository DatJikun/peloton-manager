...
### Status
Godot Watch Race is a presentation client over the existing Application clock. Open `src/Peloton.Client.Godot/project.godot` in Godot 4.4 .NET. The player confirms prep, optionally gives DS autonomy, picks a film length (default **5 min**, max 5 min), watches squad icons and a radio board interpolated from official physics, answers a knowledge-bounded decision (or the DS recommendation in parentheses), and sees the official winner from `LastRace`. Renderer does not drive physics. Career shell is not in this window. HTML lab (`08e` / `14-race.html`) stays a look reference, not the game. Headless CLI is unchanged and still uses ×1 / ×2 / ×5 / ×20. §49 remains `NOT VERIFIED`. `D-032` remains deferred. SchemaVersion remains 1.
...
2026-08-28: tablica sztabu z pozycją i radiem naszych kolarzy; widać różnicę płasko vs góra (km/h z fizyki, czerwony podjazd); film max 5 min, domyślnie 5 min; decyzja DS z tym, co chce zrobić; opcjonalna autonomia DS przed startem.
...
- `2026-08-28` — Watch: czas filmu (domyślnie 5 min, max 5 min) zamiast ×N w Godot; mapa z biblioteki profili; tablica radiowa składu; DS (chce: …) i opcjonalna autonomia DS. CLI nadal ×1/×2/×5/×20. Fizyka prototypu bez zmiany goldenu.
